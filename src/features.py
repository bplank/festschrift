...
class Featurizer:
    ...
    FW = ["an", "any", "can", "do", "every", "from","his", "may", "on","there", "upon"] #(from Tweedie et al.,1996)
    FW = FW + ["are", "is"] #extended
    ...
    def pronouns_wiktionary(self, tweet):
        if self.lowercase:
            tweet = tweet.lower()

        for word in tweet.split():
            if self.wiktionary[word] and "PRON" in self.wiktionary[word]:
                if self.binary:
                    self.d[word] =1
                else:
                    self.d[word] +=1
    ...
    def framenet_frames_all(self,tweet):
        ...
        if not self.wiktionary:
            print("call init_wiktionary before using framenet", file=sys.stderr)
